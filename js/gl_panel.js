...
// ===== WebGL 面板渲染模块（与原逻辑一致）=====
let gl, program3D, pos3DBuffer, uvBuffer, texture;
...
export function drawPanel(viewProj, model, frameImageBitmap){
  gl.useProgram(program3D);
  ...
  if(frameImageBitmap){
    ...
    gl.bindTexture(gl.TEXTURE_2D,texture);
    gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, false); // 方向由 UV 控制
